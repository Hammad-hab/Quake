...
Host.configWriteTimer = null;
Host.configAutosave = false;
Host.lastAutoSaveTime = 0.0;
Host.gameAutoSaveInterval = 15.0;

Host.CanAutoSave = function()
{
	if ((M.active_slot < 0) || (M.active_slot >= M.max_savegames))
		return false;
	if (SV.server.active !== true)
		return false;
	...
	if (client.spawned !== true)
		return false;
	if ((client.edict == null) || (client.edict.v_float[PR.entvars.health] <= 0.0))
		return false;
	return true;
};

Host.SlotSavename = function(slot)
{
	if (typeof slot === 'number')
		return 's' + slot;
	var name = String(slot).toLowerCase();
	if (name.length > 4 && name.substring(name.length - 4) === '.sav')
		name = name.substring(0, name.length - 4);
	if ((name.length === 0) || (name.charCodeAt(0) !== 115))
		return 's' + name;
	return name;
};

Host.SavegameToSlot = function(slot, quiet)
{
	if (SV.server.active !== true)
		return false;
	if (CL.state.intermission !== 0)
	...
	if (SV.svs.maxclients !== 1)
		return false;
	var client = SV.svs.clients[0];
	if ((client.active === true) && (client.edict != null) && (client.edict.v_float[PR.entvars.health] <= 0.0))
		return false;
	...
	name = COM.DefaultExtension(Host.SlotSavename(slot), '.sav');
	if (quiet !== true)
...
Host.AutoSaveSlot = function()
{
	if (Host.CanAutoSave() !== true)
		return false;
	return Host.SavegameToSlot(M.active_slot, true);
};

Host.MaybeAutoSave = function()
{
	if (Host.CanAutoSave() !== true)
		return;
	if ((Host.lastAutoSaveTime !== 0.0) && ((Host.realtime - Host.lastAutoSaveTime) < Host.gameAutoSaveInterval))
		return;
	if (Host.AutoSaveSlot() === true)
		Host.lastAutoSaveTime = Host.realtime;
};

Host.WriteConfiguration = function()
{
	if (COM.WriteTextFile('config.cfg', Key.WriteBindings() + Cvar.WriteVariables()) === true)
		Host.configDirty = false;
};

Host.ScheduleWriteConfiguration = function()
{
	if ((Host.initialized !== true) || (Host.configAutosave !== true))
		return;
	Host.configDirty = true;
	if (Host.configWriteTimer != null)
		return;
	Host.configWriteTimer = setTimeout(function()
	{
		Host.configWriteTimer = null;
		if (Host.configDirty === true)
...
Host._Frame = function()
{
	...
	Cmd.Execute();
	if ((Host.configAutosave !== true) && (Cmd.text.length === 0))
		Host.configAutosave = true;
...
Host.Shutdown = function()
{
	...
	if (Host.configWriteTimer != null)
	{
		clearTimeout(Host.configWriteTimer);
		Host.configWriteTimer = null;
	}
	Host.WriteConfiguration();
	CDAudio.Stop();
...
Host.Savegame_f = function()
{
	...
	var slot = Cmd.argv[1];
	if (slot.charCodeAt(0) === 115)
	{
		var n = parseInt(slot.substring(1), 10);
		if ((Q.isNaN(n) !== true) && (n >= 0) && (n < M.max_savegames))
			M.SetActiveSlot(n);
	}
	Host.SavegameToSlot(Cmd.argv[1], false);
};
