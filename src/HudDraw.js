...
Draw.UpdateHudScale = function()
{
	var s = Math.min(VID.width / Draw.BASEWIDTH, VID.height / Draw.BASEHEIGHT);
	if (s < 1.35)
		s = 1.35;
	else if (s > 2.25)
		s = 2.25;
...
Draw.TransformQuad = function(x, y, w, h, scaleHud)
{
	if (scaleHud === false)
		return [x, y, w, h];
	var s = Draw.hudScale;
	...
	var cy = y + h * 0.5;
	var tx, ty;
	if (cy >= VID.height * 0.75)
	{
		tx = VID.width * 0.5 + (cx - VID.width * 0.5) * s - w * s * 0.5;
		ty = VID.height - (VID.height - y - h) * s - h * s;
	}
	else if (cy <= VID.height * 0.25)
	{
		tx = x * s;
		ty = y * s;
	}
	...
	{
		tx = VID.width * 0.5 + (cx - VID.width * 0.5) * s - w * s * 0.5;
		ty = VID.height * 0.5 + (cy - VID.height * 0.5) * s - h * s * 0.5;
	}
	return [tx, ty, w * s, h * s];
};
...
{
	var t = Draw.TransformQuad(x, y, 8, 8);
	GL.StreamDrawTexturedQuad(t[0], t[1], t[2], t[3],
...
Draw.Pic = function(x, y, pic, scaleHud)
{
	var t = Draw.TransformQuad(x, y, pic.width, pic.height, scaleHud);
	var program = GL.UseProgram('Pic', true);
	GL.Bind(program.tTexture, pic.texnum, true);
	GL.StreamDrawTexturedQuad(t[0], t[1], t[2], t[3], 0.0, 0.0, 1.0, 1.0);
};

Draw.PicTranslate = function(x, y, pic, top, bottom, scaleHud)
{
	GL.StreamFlush();
...
{
	var t = Draw.TransformQuad(x, y, w, h);
	var program = GL.UseProgram('Fill', true);
	var color = VID.d_8to24table[c];
	GL.StreamDrawColoredQuad(t[0], t[1], t[2], t[3], color & 0xff, (color >> 8) & 0xff, color >> 16, 255);
};
...
Draw.BeginDisc = function()
{
	if (Draw.loadingElem == null)
		return;
	var s = Draw.hudScale;
	Draw.loadingElem.style.width = (Draw.loading.width * s) + 'px';
	Draw.loadingElem.style.height = (Draw.loading.height * s) + 'px';
	Draw.loadingElem.style.left = ((VID.width - Draw.loading.width * s) >> 1) + 'px';
	Draw.loadingElem.style.top = ((VID.height - Draw.loading.height * s) >> 1) + 'px';
