...
M.Main_Draw = function()
{
	...
	{
		M.Print((320 - (prompt[i].length << 3)) >> 1, y, prompt[i]);
		y += 8;
	}
};
...
M.SetActiveSlot = function(slot)
{
	M.active_slot = slot;
	try
	{
		localStorage.setItem('Quake.' + COM.gamedir[0].filename + '/active_slot', String(slot));
	}
	catch (e)
	{
	}
...
M.LoadActiveSlot = function()
{
	try
	{
		var slot = localStorage.getItem('Quake.' + COM.gamedir[0].filename + '/active_slot');
		if (slot != null)
		{
			slot = parseInt(slot, 10);
			if ((Q.isNaN(slot) !== true) && (slot >= 0) && (slot < M.max_savegames))
				M.active_slot = slot;
		}
	}
	catch (e)
...
M.SlotsInGame = function()
{
	return (SV.server.active === true) && (SV.svs.maxclients === 1);
};

M.SlotsCanSave = function()
{
	return M.SlotsInGame() && (CL.state.intermission === 0);
};

M.OpenSlots_f = function(mode)
{
	M.slots_mode = mode;
...
{
	var i, prompt, sx = M.slots_text_x;

	M.DrawPic(16, 4, M.qplaque);
	M.DrawPic(160 - (M.ttl_sgl.width >> 1), 4, M.ttl_sgl);
	for (i = 0; i < M.max_savegames; ++i)
		M.Print(sx, 32 + (i << 3), M.filenames[i]);
	M.DrawCharacter(sx - 8, 32 + (M.load_cursor << 3), 12 + ((Host.realtime * 4.0) & 1));

	if (M.slots_mode === 'save')
		prompt = 'Enter to save  Del to delete save';
	else if (M.slots_mode === 'load')
		prompt = 'Enter to load  Del to delete save';
	else
		prompt = 'Empty: new game  Filled: load  Auto-saves';
	M.Print((320 - (prompt.length << 3)) >> 1, 152, prompt);
};

M.SinglePlayer_SaveSlot = function(slot)
{
	if (M.SlotsCanSave() !== true)
		return;
	if ((M.loadable[slot] === true) && (slot !== M.active_slot) && (confirm('Overwrite this save?') !== true))
		return;
	S.LocalSound(M.sfx_menu2);
	M.SetActiveSlot(slot);
	M.state.value = M.state.none;
	Key.dest.value = Key.dest.game;
	Cmd.text += 'save s' + slot + '\n';
};

M.SinglePlayer_LoadSlot = function(slot)
{
	S.LocalSound(M.sfx_menu2);
	M.SetActiveSlot(slot);
	...
	Cmd.text += 'load s' + slot + '\n';
};

M.SinglePlayer_NewGameSlot = function(slot)
{
	if (SV.server.active === true)
	{
		if (confirm('Start a new game? Current game will be ended.') !== true)
			return;
		Cmd.text += 'disconnect\n';
	}
	S.LocalSound(M.sfx_menu2);
...
M.SinglePlayer_Key = function(k)
{
	...
	case Key.k.enter:
		...
		if (M.slots_mode === 'save')
		{
			M.SinglePlayer_SaveSlot(slot);
			return;
		}
		if (M.slots_mode === 'load')
		{
			if (M.loadable[slot] !== true)
				return;
			M.SinglePlayer_LoadSlot(slot);
			return;
		}
		if (M.loadable[slot] === true)
		{
			if (M.SlotsInGame() === true)
			{
				if (confirm('Load this save? Current game will be ended.') !== true)
					return;
			}
			M.SinglePlayer_LoadSlot(slot);
		}
		else
			M.SinglePlayer_NewGameSlot(slot);
		return;
	case Key.k.uparrow:
	...
	case Key.k.del:
		if (M.removable[M.load_cursor] !== true)
			return;
		if (confirm('Delete selected save?') !== true)
			return;
...
// Load/save menu (legacy commands route to slot picker)

M.ScanSaves = function()
{
	var searchpaths = COM.searchpaths, i, j, f, version, name, c, legacy;
	...
	for (i = 0; i < M.max_savegames; ++i)
	{
		...
		if (f == null)
		{
			...
			if (legacy != null)
			{
				...
				try
				{
					localStorage.removeItem('Quake.' + COM.gamedir[0].filename + '/' + i + '.sav');
				}
				catch (e)
				{
				}
...
M.Draw = function()
{
	...
	if (M.recursiveDraw !== true)
	{
		...
		{
			// DrawConsole already drew the background and text when forced up.
