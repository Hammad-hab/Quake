...
SCR.CalcRefdef = function () {
  ...
  else vrect.y = (VID.height - sbarHeight - vrect.height) >> 1;

  var viewHeight = vrect.height;
  if (full === true && Sbar.lines > 0) vrect.height = VID.height;
  ...
  if (vrect.width * 0.75 <= viewHeight) {
    ...
    R.refdef.fov_x =
      (Math.atan(
        vrect.width /
          (viewHeight / Math.tan((SCR.fov.value * 0.82 * Math.PI) / 360.0)),
      ) *
        360.0) /
      Math.PI;
...
SCR.DrawConsole = function () {
  if (Con.ui_disabled === true) return;
...
SCR.UpdateScreen = function () {
  if (SCR.disabled_for_loading === true) {
    if (Host.realtime <= SCR.disabled_time) {
      GL.Set2D();
      Draw.FadeScreen();
      Draw.String((VID.width - (7 << 3)) >> 1, VID.height >> 1, "LOADING");
      GL.StreamFlush();
      return;
    }
    SCR.disabled_for_loading = false;
  ...
  if (CL.cls.state === CL.active.connecting) SCR.DrawConsole();
    ...
    if (V.crosshair.value !== 0) {
      Draw.Character(
        ...
        R.refdef.vrect.y + (R.refdef.viewheight >> 1) + V.crossy.value,
        43,
      );
    }
    SCR.DrawNet();
