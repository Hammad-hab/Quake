...
SCR.CalcRefdef = function()
{
	...
	var sbarHeight = Math.floor(Sbar.lines * Draw.hudScale);
	if (vrect.height > (VID.height - sbarHeight))
		vrect.height = VID.height - sbarHeight;
	vrect.x = (VID.width - vrect.width) >> 1;
	if (full === true)
		vrect.y = 0;
	else
		vrect.y = (VID.height - sbarHeight - vrect.height) >> 1;

	var viewHeight = vrect.height;
	if ((full === true) && (Sbar.lines > 0))
		vrect.height = VID.height;
	R.refdef.viewheight = viewHeight;

	if (SCR.fov.value < 10)
		Cvar.Set('fov', '10');
	else if (SCR.fov.value > 170)
		Cvar.Set('fov', '170');
	if ((vrect.width * 0.75) <= viewHeight)
	{
		R.refdef.fov_x = SCR.fov.value;
		R.refdef.fov_y = Math.atan(viewHeight / (vrect.width / Math.tan(SCR.fov.value * Math.PI / 360.0))) * 360.0 / Math.PI;
	}
	else
	{
		R.refdef.fov_x = Math.atan(vrect.width / (viewHeight / Math.tan(SCR.fov.value * 0.82 * Math.PI / 360.0))) * 360.0 / Math.PI;
		R.refdef.fov_y = SCR.fov.value * 0.82;
	}

	var ymax = 4.0 * Math.tan(R.refdef.fov_y * Math.PI / 360.0);
	R.perspective[0] = 4.0 / (ymax * R.refdef.vrect.width / viewHeight);
...
SCR.DrawTurtle = function()
{
	...
	if (++SCR.count >= 3)
		Draw.Pic(R.refdef.vrect.x, R.refdef.vrect.y, SCR.turtle, false);
};

SCR.DrawNet = function()
{
	if (((Host.realtime - CL.state.last_received_message) >= 0.3) && (CL.cls.demoplayback !== true))
		Draw.Pic(R.refdef.vrect.x, R.refdef.vrect.y, SCR.net, false);
};
	...
	else
	{
		if (V.crosshair.value !== 0)
		{
			Draw.Character(R.refdef.vrect.x + (R.refdef.vrect.width >> 1) + V.crossx.value,
				R.refdef.vrect.y + (R.refdef.viewheight >> 1) + V.crossy.value, 43);
		}
		SCR.DrawNet();
		SCR.DrawTurtle();
		SCR.DrawPause();
